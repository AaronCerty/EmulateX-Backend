...
export class WebSocketSubscriptions {
   ...
   */
  async postRequest(requestType: 'info' | 'action', payload: any, timeout: number = 30000): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const responseHandler = (message: any) => {
        ...
        if (receivedMessages % 10 === 0) {
          console.log(`Received ${receivedMessages} WebSocket messages while waiting for response to request ${id}`);
        }
      ...
      setTimeout(() => {
        this.ws.removeListener('message', responseHandler);
        console.log(`Request ${id} timed out after ${timeout}ms. Received ${receivedMessages} messages.`);
